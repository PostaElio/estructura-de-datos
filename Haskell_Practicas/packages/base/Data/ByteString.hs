...
mapF a b | a `seq` b `seq` False = undefined
...
mapIndexed k (PS ps s l) = create l $ \p -> withForeignPtr ps $ \f ->
  ...
  where
    ...
    go a b c d | a `seq` b `seq` c `seq` d `seq` False = undefined
...
countEach a b c | a `seq` b `seq` c `seq` False = undefined
...
findIndexOrEnd a b | a `seq` b `seq` False = undefined
findIndexOrEnd f ps
    | null ps           = 0
    | f (unsafeHead ps) = 0
